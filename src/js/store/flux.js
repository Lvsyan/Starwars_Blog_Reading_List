  ...
  return {
      ...
      planets: [
        {
          properties: {
            ...
            terrain: "desert",
            surface_water: "1",
            created: "2022-04-19T09:01:55.620Z",
            edited: "2022-04-19T09:01:55.620Z",
            name: "Tatooine",
    ...
    actions: {
      ...
      getCharacters: async () => {
        ...
        for (let x = 0; x < data.results.length; x++) {
          const resp = await fetch(
            "https://www.swapi.tech/api/people/" + data.results[x].uid
          );
      ...
      getPlanets: async () => {
        ...
        const data = await response.json();
        const planetWithInfo = [];
        for (let x = 0; x < data.results.length; x++) {
          const resp = await fetch(
            "https://www.swapi.tech/api/planets/" + data.results[x].uid
          );
          const dat = await resp.json();
          planetWithInfo.push(dat.result);
        }
        console.log(planetWithInfo);
        setStore({ planets: planetWithInfo });
      },

      getOnePlanet: async (i) => {
        setStore({ info: {} });
      ...
      getVehicles: async () => {
        ...
        const data = await response.json();
        const vehicleWithInfo = [];
        for (let x = 0; x < data.results.length; x++) {
          const resp = await fetch(
            "https://www.swapi.tech/api/vehicles/" + data.results[x].uid
          );
          const dat = await resp.json();
          vehicleWithInfo.push(dat.result);
        }
        console.log(vehicleWithInfo);
        setStore({ vehicles: vehicleWithInfo });
      },

      getOneVehicle: async (i) => {
        setStore({ info: {} });
        const response = await fetch("https://www.swapi.tech/api/vehicles/" + i);
        const data = await response.json();
        setStore({ info: data.result });
      },
    },
